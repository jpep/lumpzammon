...
- **Phase 8.5 — Flow/screens/canvas-default** (split into sub-phases):
  ...
  - **8.5e — Canvas polish** (split):
    ...
    - [ ] **8.5e-5 — Cosmetic remnants (low priority / maybe drop).** Landscape off-zone refinement (fullscreen spike only); animating the opening dice on the canvas (opening dice are DOM today); optional bar-half perspective flip. None affect the embedded game's correctness; revisit only if desired.
