...
- **Phase 8.5 — Flow/screens/canvas-default** (split into sub-phases):
  ...
  - [ ] **8.5b — Stats wiring.** Call `storage/playerStats.appendGame` on every game end (normal/resign/forfeit), read profiles at start.
  - [ ] **8.5c — Screens.** `StatsScreen.jsx` profile overlay; restyle `LobbyScreen` with GMMN room visuals over the real Firebase lobby; merge `ModeSelect`/`BuildInfo` into the GMMN menu/About. Reconcile theming (photo-derived identity; fate of the 4 nick palettes + rainbow easter egg).
  - [ ] **8.5d — Cube + timers (net-new features).** `cube.js` reducer + offer/accept/decline UI + AI cube decision via `evaluate`; `useGameTimers.js` (per-turn + per-game, forfeit on expiry). (Optionally extract `useGameFlow.js` if the GameScreen flow needs consolidating.)
  - [ ] **8.5e — Canvas polish.** Canvas-native flying-checker/board-fill animation; bear-off **tray** rendering; landscape off-zone refinement; opening-roll dice-tap affordance; optional bar-half perspective flip.
