...
- **Phase 8.5 — Flow/screens/canvas-default** (split into sub-phases):
  ...
  - **8.5d — Cube + timers (net-new features; original React app never had these)** (split):
    - [ ] **8.5d-1 — Doubling cube.** Wire the existing `cube.js` reducer into the game: cube state (value/owner/used/promised/modal) in the React-authoritative state; the GMMN cube display + click-to-promise; offer/accept/decline modals; AI cube decision via `ai.evaluate` (`shouldAcceptDouble`); the **win-points × cubeValue multiplier** (the multiplier `gameResult.js`/8.5b deferred); online two-client handshake (mind the racy whole-object write).
    - [ ] **8.5d-2 — Timers + forfeit.** `useGameTimers.js`: per-turn move timer + per-game clock, pause during modals/animations, forfeit (opponent wins × cube) on expiry; online needs a single authoritative writer.
