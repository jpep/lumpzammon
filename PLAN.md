...
- **Phase 8.5 — Flow/screens/canvas-default** (split into sub-phases):
  ...
  - **8.5e — Canvas polish** (split):
    - [x] **8.5e-1 — Dice ball-physics roll animation.** ✅ Ported devanture's `dice.js` `ROLLING`(24f)→`SETTLING`(22f)→`DONE` ball-physics state machine into `canvas/diceAnim.js` (per-instance animator, parameterized over `(p,g,C)`; 8.4 had only the static settled faces). The two dice land **on the board in the roller's near quadrant** (the embedded square canvas has no off-board room for devanture's beside/below placement) as slightly-translucent dark squares with ivory pips that scatter then settle; a spent die dims to 50%. `sketch.js` detects a fresh roll (keyed on `turn + dice values`, so consuming a die doesn't retrigger) and `loop()`s only while animating, `noLoop()`-ing back to idle. `GameScreen` switches the canvas board to `showDice` and hides the legacy DOM turn-dice (kept for `?dom`). Dead static renderer (`drawDice`/`getDiePos`) trimmed from `dice.js` (kept `PIP_LAYOUTS` + `diceFromGameState`). **Verified (Playwright):** canvas pixels change frame-to-frame during the roll then settle to a stable image (loop stops); double + non-double rolls; `?dom` fallback still shows DOM dice; 0 console errors; 94 tests; build clean.
    - [x] **8.5e-2 — Flying-checker animation + bear-off trays.** ✅ AI moves no longer teleport on the canvas: new `canvas/flyAnim.js` slides the checker from source→destination with smoothstep easing (~0.33s), hiding the source checker mid-flight (`drawCheckers` `hideFrom`) and committing the move to the engine on the final frame (`onDone`). `sketch.js` exposes `p.animateMove(move, isWhite, onDone)` (resolves pixel endpoints from the live snapshot + geometry, handles `bar`/`off`, hit→slot 0) and loops only while flying; `GameScreen`'s AI effect drives it on the canvas (DOM `animateAndExecute` stays for `?dom`). Human drags already animate, so only AI needed it. New `canvas/drawOff.js` renders **bear-off trays** as stacked flattened bars in the board's right gutter (the embedded square's ~`bx` margin) — near player's tray at the bottom, far player's at the top (perspective-aware), capped at 15 with a count. **Verified (Playwright):** AI checker slides (canvas pixels animate *after* the dice settle, isolating the fly from the roll); AI turn completes → control returns to P1; bear-off trays render when off counts appear; 0 console errors; 94 tests; build clean.
    - [x] **8.5e-3 — Hit-to-bar fly + tap-to-roll.** ✅ **Hit-to-bar:** `flyAnim.js` detects when a fly's destination holds a lone opposing blot and slides the bumped checker from that point to the bar concurrently with the mover (a second fly), hiding both the source and the blot mid-flight (`drawCheckers` `hide` now takes an array of keys). **Tap-to-roll:** a tap on the canvas board rolls when a Roll button would be active for you — the regular roll phase **and** your pending opening roll (`sketch.js` `onRoll` opt + `view.canRoll`, prioritized in `mousePressed` so it never shadows a move drag; `CanvasBoard` `onRoll`/`canRoll` props; `GameScreen` `onRollTap`/`canRollTap` mirror the button visibility). New DEV hook `window.__gs.poke(partial)` for verifier board setup. **Verified (Playwright):** tap rolls the opening die and the turn dice; an AI hitting move animates and sends the white blot to the bar (`bar[1]===1`, black now on point 10); 0 console errors; 94 tests; build clean.
    - [x] **8.5e-4 — Online-opponent move animation.** ✅ Online opponent moves now **slide** instead of teleporting. `flyAnim.js` `diffMove(prev, next)` infers the single checker move from the pre/post engine state pair (mover = `prev.turn`; handles `bar`/`off`; returns `null` for non-single-move diffs → safe snap). `CanvasBoard`'s state-sync effect, when `animateRemoteMoves` (online) and the diff's mover ≠ me (`direction===1 ? P2 : P1`), calls the sketch's new `p.animateRemoteMove(move, isWhite, pendingView)` — which slides over the **held-back pre-move view** and swaps to the post-move view on landing (`pendingSwap`, with one extra frame so the board isn't left mid-animation). My own moves (mover === me) and rolls/cube/pass updates snap instantly; a newer state mid-fly cancels via `p.cancelFly`. Hits are picked up automatically (the fly reads the pre-move snapshot). **Verified (Playwright, two clients):** B animates A's move (canvas slides), converges to the post-move state, settles to a stable image (loop stops), and the final frame differs from the pre-move baseline; 0 console errors; 94 tests; build clean.
    - [x] **8.5e-5 — Opening dice on the canvas.** ✅ The opening rolls now animate on the board like the turn dice (8.5e-1 only did the turn roll). `sketch.js` runs two extra single-die animators (`openWhite`/`openBlack`, reusing `createDiceAnimator` via `start([v, null], owner)`); `syncOpening(gs)` starts each as its `openingRolls[p]` appears (P1 then P2) and clears them when the opening resolves / a tie resets. The draw branches on `phase==='opening'` (each player's die in its own near quadrant) vs the turn dice; `GameScreen` hides the DOM opening dice when on the canvas (kept for `?dom`). **Verified (Playwright):** the opening die animates on the canvas (frame-diff) and renders in white's near quadrant; the AI rolls its die; the opening resolves; 0 console errors; 94 tests; build clean. **Dropped (no correctness impact, not worth the churn):** landscape off-zone refinement (fullscreen `?canvas` spike only) and the optional bar-half perspective flip (no actual rendering bug — online P2 perspective verified in 8.5a). **8.5e complete.**
