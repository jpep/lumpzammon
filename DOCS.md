...
### Phase 8.5e-4 — Online-opponent move animation (done)

Online opponent moves now **slide** instead of teleporting. Unlike the AI path (where `GameScreen` drives `animateMove` and commits in `onDone`), a remote move's new state **already exists** when it arrives via the Firebase subscription — so the work is to *infer* what moved and *delay* showing the new state until the slide lands.

- **`flyAnim.js` `diffMove(prev, next)`** — infers the single checker move from the pre/post engine state pair. The mover is whoever was on turn in `prev`; it scans the mover's per-point counts for a single `-1` (source) and `+1` (dest), plus `bar[mover]-1` → `'bar'` source / `off[mover]+1` → `'off'` dest. Anything that isn't a clean single move (a roll, a cube change, a multi-step combined hop, a pass) returns `null` → the caller snaps instead of animating. (A hit needs no special handling — the fly reads the pre-move snapshot and bumps the blot itself.)
- **`CanvasBoard`** — the state-sync effect now keeps the previous `gameState` and, when `animateRemoteMoves` (online) is set and `diffMove`'s mover ≠ me (`direction===1 ? P2 : P1`, so my **own** echoed move snaps), calls `inst.animateRemoteMove(move, isWhite, newView)` instead of `inst.update(newView)`. A newer state arriving mid-fly cancels it (`inst.cancelFly`) and snaps.
