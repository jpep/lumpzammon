...
- **Strata stacks (experimental, local test)** — gated by the `STRATA_STACKS` flag at the top of `sketch.js`. When a player has **2 or more checkers on a point**, the individual checkers are not drawn; instead the triangle fills with the player's colour (offwhite/ruby) **stratum by stratum from the base**. The staircase's 7 `TRI_LAYERS` are grouped into **5 strata** (`STRATA_LAYER_GROUPS`: base block = layers 0+1, one stratum per middle step, tip = layers 5+6): 2 pieces = 2 strata, 3 = 3, 4 = 4, **5 or more = full triangle** (counts above 5 are intentionally indistinguishable). Single checkers (blots) and the bar keep the classic round rendering; pickup hit-zones are unchanged (click where the pile would be), and the game-start fade-in animates stratum by stratum at the same cadence as the checkers it replaces. Set the flag to `false` to restore the classic stacked-checkers rendering.
