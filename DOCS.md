...
- **Deferred to later 8.5 sub-phases:** flying-checker/board-fill animation (moves snap; AI moves snap because the DOM `animateAndExecute` finds no `[data-point-id]` and fires its callback immediately); `playerStats.appendGame` wiring on game end; `StatsScreen` + `LobbyScreen` restyle + menu/About merge; doubling cube; timers; bear-off **tray** rendering; landscape off-zone refinement.
