...
- **Deferred to later 8.5 sub-phases:** flying-checker/board-fill animation (moves snap; AI moves snap because the DOM `animateAndExecute` finds no `[data-point-id]` and fires its callback immediately); `StatsScreen` + `LobbyScreen` restyle + menu/About merge; doubling cube; timers; bear-off **tray** rendering; landscape off-zone refinement.
...
- **`app/src/game/gameResult.js`** — pure `gameEndResult({ gs, winner, isOnline, isAI, playerSlot, opponentName })` → `{ youScore, oppScore, opponent, delta, didWin }` or `null`. Identity: vs-AI the human is P1; online it's `playerSlot`; **local two-player returns `null`** (no single identity to attribute on one device). `delta = ±winPoints(classifyWin(gs, winner))` — **gammon/backgammon aware** (simple 1 / gammon 2 / backgammon 3). The doubling-cube and resign multipliers are deferred to 8.5d (no cube exists in the engine yet). 10 unit tests.
